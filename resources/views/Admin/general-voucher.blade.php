...
@section('content')
    <div class="page-wrapper">
        <div class="content container-fluid">
            <div class="mb-3">
                <h6 class="ps-2">General Voucher</h6>
            </div>
                                ...
                                <select class="form-select" id="ledgerId">
                                    ...
                                    @foreach ($ledgers as $ledger)
                                        <option value="{{ $ledger->Account_Id }}">
                                            {{ $ledger->Account_Code }} - {{ $ledger->Account_Desc }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.editRow', function() {
                ...
                $.get("{{ url('general-voucher/details') }}/" + id, function(d) {
                    $('#vouchId').val(d.Voucher_Id);
                    $('#voucherNo').val(d.Vou_No);
                    $('#vouNoDiv').show();
                    $('#voucherDate').val(d.Vou_Date);
                    $('#voucherType').val(d.Vou_Type);
                    $('#ledgerId').val(d.Ledger_Id).trigger('change');
                    $('#amount').val(d.Amount);
            ...
            $('#saveVoucher').on('click', function() {
                ...
                if (!$('#voucherType').val()) {
                    Swal.fire('Error', 'Please select Voucher Type', 'error');
                    return;
                }
                if (!$('#ledgerId').val()) {
                    Swal.fire('Error', 'Please select a Ledger', 'error');
                    return;
                }
                ...
                $.ajax({
                    url: "{{ route('general-voucher.store') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
        ...
        function resetForm() {
            ...
            $('#voucherType').val('');
            $('#ledgerId, #bankAccountId').val('').trigger('change');
